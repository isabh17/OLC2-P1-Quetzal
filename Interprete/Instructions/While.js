  ...
  execute(tree, table){
    ...
    while (true){
      ...
      if (this.condition.type === Type.BOOLEAN){ // Aqui verifica si la condition es una expresion logica, sino lanza una Exception.
        ...
        //tree.removeAmbito();
        ErrorList.addError(new ErrorNode(this.row,this.column,new ErrorType(EnumType.SEMANTIC),`Error en while, la expresion no retorna un booleano.`,ENVIROMMENT.IF));
